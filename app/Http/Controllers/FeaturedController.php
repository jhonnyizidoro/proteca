...
class FeaturedController extends Controller
{
	public function showFeatured()
	{
		...
		return view('admin.featured')->with('featuredVideos', $featuredVideos)->with('featuredPosts', $featuredPosts)->with('mainFeaturedVideo', $mainFeaturedVideo);
	}
	...
	public function createFeaturedVideo(NewFeaturedVideoRequest $request)
	{
		if (!$title = self::getYoutubeTitleFromUrl($request->video_url)) {
			return redirect()->back()->with('status', 'O link para o vídeo informado é invalido. Certifique-se de que se trata de um vídeo do youtube.');
		}
		if ($request->main == 1) {
		...
		return redirect()->back()->with('status', 'Vídeo em distaque adicionado com sucesso.');
	}
